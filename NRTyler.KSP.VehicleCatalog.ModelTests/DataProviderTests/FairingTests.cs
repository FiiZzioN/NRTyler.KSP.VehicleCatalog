...
namespace NRTyler.KSP.VehicleCatalog.ModelTests.DataProviderTests
{
	...
	public class FairingTests
	{
		...
		public void FairingConstructorOne()
		{
			...
			var expected = new List<double?> { null, null };

			//Act
			var actual = new List<double?> {  fairing.Diameter, fairing.Length };
		...
		{
			//Arrange instantiation
			var fairing  = new Fairing(2.5, 12.2);
			var expected = new List<double?> { 2.5, 12.2};
		...
		{
			//Arrange instantiation
			var fairing  = new Fairing(-1.1, -5.1);
			var expected = new List<double?> { null, null };
